	...
	return (
		<div className="relative w-full md:w-2/3 h-full">
			<input
				...
				className="peer placeholder:text-neutral-500 text-neutral-950 dark:text-neutral-50 w-full px-4 py-3 md:text-lg rounded-lg bg-white border border-neutral-50 shadow-xl dark:bg-neutral-900 dark:border-neutral-800 shadow-black/5 dark:shadow-none outline-none"
			/>
				...
				{searchButtonTransition((style, item) =>
					item ? (
						<a.div style={style} className="h-full">
							...
							>
								<Select.Trigger
									className="group relative inline-flex items-center justify-between h-full w-12 hover:w-44 data-[state='open']:w-44 px-3 duration-300 overflow-clip ease-in-out outline-none"
									aria-label="Search engine"
								>
									<Select.Value />
									<Select.Icon className="absolute right-3 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 group-data-[state='open']:opacity-100 duration-200">
										<svg
												...
												>
													<Select.ItemText asChild>
														<div className="flex gap-3 items-center">
															...
															<span className="group-data-[state='closed']:opacity-0 group-data-[state='closed']:group-hover:opacity-100 duration-200">
																{engine.name}
															</span>
														</div>
													</Select.ItemText>
