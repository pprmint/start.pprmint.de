...
function Display({ digit }: { digit: keyof typeof Patterns }) {
	const segments = Patterns[digit];
	return (
		<svg width="100%" height="100%" viewBox="0 0 120 200" version="1.1" xmlns="http://www.w3.org/2000/svg">
			<path
...
export default function LCDClock({ time, meridiem, large }: { time: string; meridiem?: "AM" | "PM"; large?: boolean }) {
	const digits = useMemo(() => time.split(""), [time]);
	return (
		<div className={`flex items-center gap-3 h-28 ${large && "lg:h-40 lg:gap-4"} duration-700 ease-in-out`}>
			{digits.map((digit, index) =>
				digit === ":" ? (
					<div className="flex flex-col h-full justify-evenly">
						<div
							className={`${large ? "size-4" : "size-3"} bg-accent rounded-full duration-700 ease-in-out`}
						/>
						<div
							className={`${large ? "size-4" : "size-3"} bg-accent rounded-full duration-700 ease-in-out`}
						/>
					...
					<Display digit={parseInt(digits[index]) as keyof typeof Patterns} />
				)
			)}
