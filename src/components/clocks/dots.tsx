		...
		<div className={`relative size-64 ${large && "lg:size-96"} rounded-full duration-700 ease-in-out`}>
			<div className="absolute inset-0">
				{[...Array(60)].map((_, index) => (
					<div
						key={index}
						className={`absolute inline-flex items-center justify-center top-0 left-1/2 size-3 origin-[6px_128px] ${
							large && "lg:origin-[6px_192px]"
			...
			>
				<div
					className={`absolute bg-foreground-2 w-2 h-12 top-16 ${
						large && "lg:h-20 lg:top-24"
					} left-1/2 -translate-x-1/2 rounded-full duration-700 ease-in-out`}
				/>
			</div>
			<div
				id="minute"
			...
			>
				<div
					className={`absolute bg-foreground-2 w-2 h-24 top-4 ${
						large && "lg:h-39 lg:top-5"
					} left-1/2 -translate-x-1/2 rounded-full duration-700 ease-in-out`}
				/>
			</div>
			{time[2] !== undefined && time[2] !== null && (
				...
				>
					<div
						className={`absolute bg-accent w-2 h-24 top-4 ${
							large && "lg:h-39 lg:top-5"
						} left-1/2 -translate-x-1/2 rounded-full duration-700 ease-in-out`}
					/>
				</div>
